class NovelDisplay{
    ...
    //1文字列表示後に自動改行するかどうか
    private isAutoNewLine: boolean;

    //文字列二重表示可能判定
    public isEnableSendText: boolean;

    constructor(canvas: HTMLCanvasElement) {

        this.canvas = canvas;

        this.canvas2D = <CanvasRenderingContext2D>canvas.getContext("2d");
        //位置の初期化
    ...
    public showOneString(str: string) {
    ...
    //一文字を表示
    private showOnechar(oneChar: string) {

        //次の文字を表示した場合の幅を取得
        var currentWidth = this.canvas2D.measureText(
            this.sentences[this.currentPage][this.currentRow] + oneChar).width;
